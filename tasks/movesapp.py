...
# TODO: NEED to understand Moves' notifications to know how to handle updating
...
def last_update_datetime(profile, record_type):
    ...
    return last_update.replace(tzinfo=pytz.utc)
...
def next_import_date_range(last_update):
    ...
    """
    offset = (datetime.datetime.now(pytz.utc) - last_update).days
...
def import_resource_type(profile, record_type):
    ...
    if update_info:
        return update_resource(profile, record_type, update_info)
